...
export default function RideDetail({ ride, onBack, onContact, bookingSuccess = false, onReturnHome }: RideDetailProps) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* Botón para terminar el viaje (solo conductor y si no está completado) */}
      {currentUserId === ride.driver_id && !ride.is_completed && (
        <button
          onClick={async () => {
            try {
              await fetch(`/api/rides/${ride.id}/complete`, {
                method: 'POST',
          ...
          className="w-full bg-green-600 text-white px-6 py-4 rounded-xl font-semibold hover:bg-green-700 transition-colors flex items-center justify-center space-x-2 shadow-lg mt-4"
        >
          <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
            <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-12a1 1 0 10-2 0v4a1 1 0 00.293.707l2.828 2.829a1 1 0 101.415-1.415L11 9.586V6z" clipRule="evenodd" />
          </svg>
